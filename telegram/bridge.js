...
class TelegramBridge {
    constructor(whatsappBot) {
        ...
        this.tempDir = path.join(process.cwd(), 'temp'); // Use process.cwd() for relative paths in ESM
    ...
    async initialize() {
        ...
        try {
            ...
            const { default: TelegramCommands } = await import('./commands.js');
    ...
    // --- LID/PN JID RESOLUTION UTILITY (CRITICAL) ---
    async resolveJidDetails(jid, msgKey = {}) {
        const isGroup = jid.endsWith('@g.us');
        let phone = '';
        let resolvedJid = jid;

        try {
            // 1. Check for alternate JID in MessageKey (Baileys v6.8.0+ feature)
            const altJid = isGroup ? msgKey.participantAlt : msgKey.remoteJidAlt;
            if (altJid) {
                resolvedJid = altJid;
            }
        } catch (e) {
             logger.debug(`Could not resolve altJid for ${jid}: ${e.message}`);
        }
        
        // 2. Try to get PN from the LID mapping store if the JID looks like an LID
        if (jid.includes('@lid') && this.whatsappBot?.sock?.signalRepository?.lidMapping?.getPNForLID) {
            const pnJid = await this.whatsappBot.sock.signalRepository.lidMapping.getPNForLID(jid);
            if (pnJid) {
                resolvedJid = pnJid; // Use the PN JID
            }
        } else if (resolvedJid.includes('@lid') && this.whatsappBot?.sock?.signalRepository?.lidMapping?.getPNForLID) {
            const pnJid = await this.whatsappBot.sock.signalRepository.lidMapping.getPNForLID(resolvedJid);
            if (pnJid) {
                resolvedJid = pnJid; // Use the PN JID
            }
        }

        // 3. Extract phone number from the resolved JID
        phone = resolvedJid.split('@')[0];

        // 4. Get the contact name
        const contactName = this.contactMappings.get(phone);

        return { jid: jid, resolvedJid: resolvedJid, phone: phone, name: contactName };
    }
    ...
    async syncContacts() {
        try {
            ...
            for (const [jid, contact] of contactEntries) {
                ...
                const details = await this.resolveJidDetails(jid);
        ...
        try {
            ...
            for (const [jid, topicId] of this.chatMappings.entries()) {
                if (!jid.endsWith('@g.us') && jid !== 'status@broadcast' && jid !== 'call@broadcast') {
                    ...
                    const details = await this.resolveJidDetails(jid);
                    const phone = details.phone;
                    const contactName = details.name;
    ...
    async syncMessage(whatsappMsg, text) {
        ...
        if (whatsappMsg.message?.ptvMessage || (whatsappMsg.message?.videoMessage?.ptv)) {
            ...
            if (sender.endsWith('@g.us') && participant !== sender) {
                const details = await this.resolveJidDetails(participant, whatsappMsg.key);
                const senderPhone = details.phone;
                const senderName = details.name || senderPhone;
    ...
    async handleStatusMessage(whatsappMsg, text) {
        try {
            ...
            const details = await this.resolveJidDetails(participant, whatsappMsg.key);
            const phone = details.phone;
            const contactName = details.name || `+${phone}`;
    ...
    async createUserMapping(participant, whatsappMsg) {
        const details = await this.resolveJidDetails(participant, whatsappMsg.key);
        const userPhone = details.phone;
        const participantJid = participant; 

        if (this.userMappings.has(participantJid)) {
            const userData = this.userMappings.get(participantJid);
            userData.messageCount = (userData.messageCount || 0) + 1;
            await this.saveUserMapping(participantJid, userData);
            return;
        }
        ...
        const creationPromise = (async () => {
            ...
            try {
                ...
                if (isStatus) {
                    ...
                    const details = await this.resolveJidDetails(chatJid, whatsappMsg.key);
                    const phone = details.phone;
                    const contactName = details.name;
    ...
    async sendWelcomeMessage(topicId, jid, isGroup, whatsappMsg, initialProfilePicUrl = null) {
        try {
            ...
            const details = await this.resolveJidDetails(jid, whatsappMsg.key);
            const phone = details.phone;
            const contactName = details.name || `+${phone}`;
    ...
    async handleCallNotification(callEvent) {
        ...
        try {
            const details = await this.resolveJidDetails(callerId);
            const phone = details.phone;
            const callerName = details.name || `+${phone}`;
        ...
        const sendMedia = async (finalTopicId) => {
            try {
                ...
                if (isOutgoing) caption = caption ? `📤 You: ${caption}` : '📤 You sent media';
                    ...
                    const details = await this.resolveJidDetails(whatsappMsg.key.participant, whatsappMsg.key);
                    const senderPhone = details.phone;
                    const senderName = details.name || senderPhone;
                    caption = `👤 ${senderName}:\n${caption || ''}`;
                }
    ...
    async handleWhatsAppContact(whatsappMsg, topicId, isOutgoing = false) {
        try {
            ...
            const phoneNumber = contactMessage.vcard.match(/TEL.*:(.*)/)?.[1]?.replace(/[^0-9+]/g, '') || '';
            const sender = whatsappMsg.key.remoteJid;
    ...
    async handleStatusReply(msg) {
        try {
            ...
            const details = await this.resolveJidDetails(statusJid);
            const contactName = details.name || `+${details.phone}`;
            ...
            await this.telegramBot.sendMessage(msg.chat.id, `❌ Failed to send reply to contact`, {
                message_thread_id: msg.message_thread_id
            });
            await this.setReaction(msg.chat.id, msg.message_id, '❌');
        }
    ...
    async handleTelegramSticker(msg) {
        ...
        try {
            ...
            const fileLink = await this.telegramBot.getFileLink(fileId);
            stickerBuffer = (await axios.get(fileLink, { responseType: 'arraybuffer' })).data;
            ...
            try {
                ...
                for (const contact of contacts) {
                    if (contact.id && contact.name) {
                        const details = await this.resolveJidDetails(contact.id);
                        const phone = details.phone;
    ...
    async verifyTopicExists(topicId) {
        if (this.topicVerificationCache.has(topicId)) return this.topicVerificationCache.get(topicId);

        try {
            const chatId = config.get('telegram.chatId');
            
            await this.telegramBot.sendChatAction(chatId, 'typing', { message_thread_id: topicId });
            this.topicVerificationCache.set(topicId, true);
            setTimeout(() => this.topicVerificationCache.delete(topicId), 60000);
            return true;

        } catch (error) {
            const desc = error.message;
            if (desc.includes('message thread not found') || desc.includes('not a member')) {
                this.topicVerificationCache.set(topicId, false);
                return false;
            }
            logger.debug(`Error during topic existence check (Topic ID: ${topicId}): ${desc}`);
            this.topicVerificationCache.set(topicId, true); 
            return true;
        }
    }
